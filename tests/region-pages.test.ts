...
// Region-specific indexable pages: /au /uk /sg /ca /de, reusing the homepage's
...
test("a route file exists for every non-US region", () => {
  for (const [country, dir] of [["AU", "au"], ["UK", "uk"], ["SG", "sg"], ["CA", "ca"], ["DE", "de"]] as const) {
...
test("every region page's metadata self-references its own canonical and carries the full hreflang set", () => {
  for (const country of ["AU", "UK", "SG", "CA", "DE"] as const) {
