...
test("/login converts ?src= into stashed attribution without inflating the click metric", () => {
  ...
  assert.match(form, /const stashed = \(source \?\? urlSrc\) \? null : readSignupSource\(\);\s*const placement = source \?\? urlSrc \?\? stashed \?\? "login";\s*if \(stashed\) stashSignupSource\(stashed\);\s*else markSignupSource\(placement\);/);
