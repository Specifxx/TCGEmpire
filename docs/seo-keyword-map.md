...
| `riftbound deck builder`, `riftbound deck price`, `riftbound deck cost` | `/deck` (deck pricer/builder) | `/decks` for published lists |
| `riftbound decks`, `riftbound decklists`, `<champion> decks riftbound` | `/decks` (player-published deck library, 2026-09-26) and `/decks/legend/<champion>` per legend; each `/decks/<slug>` owns its own deck ("{Legend} deck — US$X to build"). Player-published only — this does not change the unowned meta/tier-list row below | `/deck` (build/price your own); `/guides/riftbound-deck-archetypes-guide`, `/guides/budget-riftbound-decks` |
...
| `riftbound heartsteel overnumbered`, `heartsteel overnumbered`, `radiance heartsteel cards`, `k'sante courageous`, `ksante courageous` | `/blog/riftbound-heartsteel-overnumbered-cards` (added 2026-09-26, the day Riot's official six-card graphic surfaced) — the **reveal** intent for the whole HEARTSTEEL sheet: RAD 178–183/167, which five are reprints (cross-checked against the OGN/SFD originals) and the one new card, K'Sante, Courageous. A SIBLING to the HEARTSTEEL post, not a rewrite: that post answers the music-video tease it was written for and now links here from a dated update | `/blog/riftbound-heartsteel-cards` keeps `riftbound heartsteel` and the band / skin-line intent; `/blog/riftbound-ksante-radiance-spoiler` keeps K'Sante's debut (`riftbound ksante`, `pride of nazumah`). Title omits "Radiance" + "spoiler" (the tracker owns that pair) and names the PRINTING, "Overnumbered", which is the part no other page answers. The six `/card/*-rad-17x/18x-167` pages keep their own `<card name> price` queries per this file's standing rule; when the official card list lands and the cards have prices, the card pages and `/sets/radiance` take over and this post is a candidate to fold into the HEARTSTEEL post |
