...
| `buy_click` | Any outbound retailer link (`OutboundLink`) is clicked, sitewide | `retailer`, `country`, `kind`, `card_id`, `card_name`, `price`, `position_in_list`, `page_type`, `in_stock`, `variant`, `condition`, `surface`, `entry` — all except `retailer`/`country`/`kind` are optional and populated only where the calling component already has the data. `entry` (2026-09-26) is how the tab's session first arrived: `reddit` \| `discord` \| `search` \| `social` \| `email` \| `internal` \| `direct` \| `other` (`src/lib/entry-source.ts`). `surface` names the placement: `table` \| `modal` \| `out_of_stock` \| `shop_strip_inline` \| `shop_strip_end`, and for eBay units outside a ranked table (2026-09-26, DECISIONS "Pushing eBay clicks") `ebay_panel` \| `ebay_search` \| `ebay_fallback` \| `ebay_cta` \| `ebay_banner` \| `ebay_picks` \| `ebay_carousel` \| `ebay_graded` \| `preorder_cta_ebay` \| `cheapest_ebay` \| `search_box` |
| `preorder_cta_click` | The Radiance pre-order CTA on an article or `/sets/radiance` is clicked through to `/radiance-preorders` (2026-09-26) | `placement` (`top` \| `section`), `has_price`, `entry` |
