...
| 10 | `/guides/riftbound-spiritforged-card-list` + Origins/Proving Grounds/Unleashed card-list pages | 4 | 3 | 3 | 🟡 Origins done 2026-08-13 | Shipped `/guides/whats-in-the-riftbound-origins-set` — real rarity/domain/type breakdown (298 base cards, 88/84/84/42 by rarity) computed from `prisma/riftbound-cards.json`, not a full card-by-card list like the Vendetta template. Proving Grounds/Spirit Forged/Unleashed still not attempted — same data source would support them (222 SFD base cards, 225 UNL, 24 OGS, all reconciled against the seed's collector numbers), deferred on scope for this pass. |
