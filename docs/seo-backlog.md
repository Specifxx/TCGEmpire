...
| 10 | `/guides/riftbound-spiritforged-card-list` + Origins/Proving Grounds/Unleashed card-list pages | 4 | 3 | 3 | ✅ Origins 2026-08-13; Spirit Forged + Unleashed 2026-09-10 | Shipped `/guides/whats-in-the-riftbound-origins-set` — real rarity/domain/type breakdown (298 base cards, 88/84/84/42 by rarity) computed from `prisma/riftbound-cards.json`, not a full card-by-card list like the Vendetta template. Same template now shipped for `/guides/whats-in-the-riftbound-spirit-forged-set` (221 base, 60/63/60/38) and `/guides/whats-in-the-riftbound-unleashed-set` (219 base, 60/63/60/36), each with the full Showcase/Signature/over-numbered accounting, the 12 Legends linked to live card + champion pages, and the energy curve. NOTE the earlier "222 SFD / 225 UNL" figures in this row were wrong: the JSON carries token cards (`sfd-t03`, `unl-t01`…`unl-t07`) that share a collector number with a base card and must be excluded — the printed denominators (221/219) are the truth. Proving Grounds (24 cards) is too small to carry a 1500-word guide honestly; folded into the sets-in-order guide instead. |
| 11 | Riftbound set roadmap | 3 | 4 | 3 | ✅ already existed, doc was stale | This row referenced a slug (`/blog/riftbound-set-roadmap-2026`) that was never real — the actual article shipped separately as **`/blog/riftbound-2027-set-roadmap`** (2026-08-04, sourced from Riot's own 4 Aug announcements). Corrected in `docs/seo-keyword-map.md`; this backlog entry was simply never updated after that post landed. |
| 12 | `/guides/riftbound-card-list` all-sets hub | 3 | 1 | 3 | ✅ done 2026-08-13 | Shipped as `/guides/riftbound-sets-in-order` — a narrative release-order guide rather than a bare link hub (real card counts per set, released sets covered in full, future sets deliberately deferred to the 2027 roadmap post rather than restated, to avoid the two drifting apart if Riot's dates move). Serves the same query intent as originally scoped. |
...
| 14 | `/sealed/accessories` + `/guides/riftbound-playmats` | 3 | 3 | 3 | 🟡 editorial half shipped 2026-09-12 | Real trend signal (playmats/sleeves/deck boxes rising). Would need either (a) new `Product`-adjacent inventory in the existing sealed-import pipeline for accessory SKUs, or (b) a lighter editorial-only page linking out to affiliate searches (same pattern as the "SEALED_SEARCHES" eBay/Amazon cross-sell block already on `/sealed`). (b) is buildable without new infra; deferred on time budget, not blocked. **Update 2026-09-12**: the editorial half shipped as `/guides/riftbound-card-size-sleeves-deck-boxes` — the accessory hub (card size, sleeve counts from the tracked tournament lists, deck-box capacity, binder page maths, playmats) with an eBay `shop` strip for sleeves/boxes/binders. Playmats are a SECTION there, not the separate `/guides/riftbound-playmats` page this row scoped: the honest answer is "standard size, buy what you like", which does not carry a page. The `/sealed/accessories` half — real accessory SKUs in the sealed-import pipeline — is still not attempted and still needs the inventory work. |
