...
// DATABASE_URL_2 → RM6 → RM7 → RM8 → RM9 → RM10), where each project's OPERATIONAL_VARS
...
//
// RM10 IS A RECYCLED NAME, not a new project — it was live once already
// (2026-08-26..~08-29, before rotating to RM11) and has been sitting idle since,
...
// evidence that its 2026-08-26..08-29 window was already carried forward, and
...
// database. The operational variable is RM10, so anything that runs Prisma MUST
// copy RM10 into DATABASE_URL first or it will talk to whatever DATABASE_URL
// happens to hold while the app talks to RM10. scripts/build-db-push.sh does
...
//
// RM3 through RM11 (bar RM10 itself) and DATABASE_URL_2 are retired and reachable
...
// distinguishes "RM10 is down" from "RM10 is unset in this environment".
...
if (OPERATIONAL_URL_SOURCE !== "RM10") {
  console.warn(
