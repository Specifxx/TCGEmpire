...
function coverageNotes(c: NarrativeInput): string | null {
  ...
  if (stocked.length === 1 && c.baseline.storeCount > 0) {
    bits.push(
      `${stocked[0].place} is currently the only one of our six markets with a copy in stock — Australia, ` +
        `the US, the UK, Singapore, Canada and Germany are all checked daily, and the others have nothing listed today`,
    );
  }
...
function noMarketYet(c: NarrativeInput): string {
  ...
  let s2 =
    ...
    `Singapore, Canada or Germany currently has it on the shelf.`;
  if (otherPriced.length) {
    const cheapest = otherPriced.reduce((a, b) => ((b.priceCents ?? 0) < (a.priceCents ?? 0) ? b : a));
    s2 +=
