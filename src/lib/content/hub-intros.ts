...
export const HUB_INTROS: Record<string, HubIntro> = {
  ...
  "/tools/best-basket": {
    paragraphs: [
      ...
      "Postage is each store's own checkout rate, measured for orders of different sizes and values to addresses across your market — the rate's own name, whether its name says it is tracked, where a cheap untracked letter stops being offered, and where postage goes free. Your region is priced at the address we measured there (in the US: New York, Chicago, Dallas or San Francisco); unset, each store is priced at its highest. A store we have not measured yet is marked as an estimate, and the store's checkout is always final.",
    ],
  },
  "/tools/demand": {
    paragraphs: [
