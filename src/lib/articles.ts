...
**Global composite.** The default view blends every region's own Index (computed exactly as above, in that region's own currency) into one currency-agnostic number, chain-linked the same way a region chain-links its own constituents: each region's index level stands in for a constituent's price, equal weight for every region. A region with no reading yet simply isn't part of the return for the step it joins on, so the composite starts as early as ANY region's own tracked history goes back - not held hostage to whichever region has been live for the shortest time - and a region coming online later still can't jump it.
