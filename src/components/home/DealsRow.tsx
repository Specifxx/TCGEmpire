...
// priceDrops, cheapestSealed, undervalued) unchanged — this file only
// SELECTS from data already computed elsewhere, it doesn't add new pricing
// aggregation. Filter chips are gone entirely (they now belong to the deals
// page they filter, not the homepage teaser of it).
const KIND_LABEL: Record<DealType, string> = {
  "price-drops": "Price drop",
  "cheapest-sealed": "Cheapest sealed",
  "savings-vs-market": "eBay deal",
  undervalued: "Below average",
};

// Free signals FIRST. The old per-column gating on the homepage revealed
// exactly one item from each Premium signal (savingsVsMarket, undervalued)
// and the rest in full for the free ones (priceDrops, cheapestSealed) — see
// the retired TodaysTopDeals.tsx. Round-robin-ing this order preserves that
// same exposure level without re-implementing a locked-teaser UI in a
// six-card row: the first pass takes one item per signal (one free preview
// from each Premium column, same as before), and only a SECOND pass — which
// only ever reaches the free signals, since they're ordered first — pulls in
// the extra cards needed to fill out to six.
const SIGNAL_ORDER: (keyof Omit<TopDeals, "hasAny">)[] = ["priceDrops", "cheapestSealed", "savingsVsMarket", "undervalued"];
const ROW_SIZE = 6;
