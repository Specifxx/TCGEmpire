...
export function MyCollection() {
  ...
  async function patch(id: string, body: Record<string, unknown>) {
    ...
    try {
      ...
      if (data.merged) {
        // The edited row folded into an existing (card,condition,foil) row — refetch
        // so quantities and the merged target are accurate.
  ...
  return (
    <div className="card-surface mt-5 p-5">
      <div className="flex flex-wrap items-center justify-between gap-3">
        <div>
          <h2 className="font-bold text-white">My Cards</h2>
          <p className="text-sm text-slate-400">
            {items == null
              ? "Loading…"
              : items.length === 0
              ? "Your collection is empty."
              : `${summary.distinct} ${summary.distinct === 1 ? "card" : "cards"} · ${summary.total} total${
                  summary.priced ? ` · ~${fmt(summary.value)} value` : ""
                }`}
          </p>
        </div>
        <Link href="/scan" className="btn-primary">📷 Scan a card →</Link>
      </div>

      {items != null && items.length === 0 && (
        <p className="mt-4 text-sm text-slate-500">
          Scan cards with the <Link href="/scan" className="text-brand-400 hover:underline">Card Scanner</Link> and tap
          “+ My Cards” to build your collection here — track quantities, conditions and live value.
        </p>
      )}
              ...
              <li key={it.id} className="flex items-center gap-3 py-3">
                ...
                <div className="min-w-0 flex-1">
                  ...
                  <div className="mt-0.5 flex flex-wrap items-center gap-1.5 text-xs text-slate-500">
                    ...
                    {it.isFoil && <span className="chip bg-sky-500/15 text-sky-300">Foil</span>}
                  ...
                  <div className="mt-2 flex flex-wrap items-center gap-2">
                    ...
                    <button
                      ...
                      disabled={busy === it.id}
                      className={`rounded-md px-2 py-1 text-xs font-medium ${it.isFoil ? "bg-sky-500/20 text-sky-300" : "bg-ink-800 text-slate-400 hover:text-slate-200"}`}
                    >
                      Foil
                    </button>
                    ...
                    <div className="flex items-center overflow-hidden rounded-md border border-ink-700">
                      <button
                        onClick={() => patch(it.id, { quantity: Math.max(0, it.quantity - 1) })}
                        disabled={busy === it.id}
                        className="px-2 py-1 text-sm text-slate-300 hover:bg-ink-800"
                        aria-label="Decrease quantity"
                      >−</button>
                      <span className="min-w-8 px-2 text-center text-sm font-semibold text-white">{it.quantity}</span>
                      <button
                        onClick={() => patch(it.id, { quantity: Math.min(999, it.quantity + 1) })}
                        disabled={busy === it.id}
                        className="px-2 py-1 text-sm text-slate-300 hover:bg-ink-800"
