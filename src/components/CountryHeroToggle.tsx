...
//
// UNLIKE CountrySwitcher (the navbar one), this toggle only ever appears on
// the homepage / region home pages, so a pick here ALSO navigates to that
// market's own crawlable URL (REGION_HOME_PATH — "/" for US, "/au" etc.
// otherwise) rather than only mutating the client-side country cookie.
// CountrySwitcher stays URL-stable on purpose: switching market on a
// card/browse/set page must re-price that SAME page, not navigate away from
// it. No separate analytics call here — CountryProvider's setCountry() is
// the one shared choke point every region control (this one, CountrySwitcher,
// the marketplace pickers) already funnels through, and it fires
// `region_changed` itself; a second event here would just double-count every
// hero-triggered switch.
//
          ...
          {COUNTRY_LIST.map((c) => (
            <button
              ...
              onClick={() => {
                ...
                // c.code === country, so this only needs to close the panel
                // and, since this control alone represents a real, crawlable
                // per-market URL, navigate there.
                setCountry(c.code);
                setOpen(false);
