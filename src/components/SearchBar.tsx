...
export function SearchBar({
  ...
  function markInitiated(trigger: "keystroke" | "focus_dwell") {
    ...
    trackEvent("search_initiated", { trigger, variant });
  }
    ...
    const t = setTimeout(async () => {
      ...
      try {
        ...
        if (nextResults.length === 0 && nextSealed.length === 0) {
          trackEvent("search_no_results", { query: q, variant });
        }
      } catch {
  ...
  // functionally, resubmitting that exact past query). `resultsCount`, when
  // passed, is the preview dropdown's count AT THE MOMENT OF SUBMIT — folded
  // in as enrichment on search_submitted rather than a second `card_search`
  // event (an earlier, independent pass at this file built exactly that;
  // reconciled the same way as OutboundLink's buy_click/store_click overlap —
  // one event per real action, not two, see DECISIONS.md). Only the direct-
  // submit path has a real count to give (the debounced fetch has resolved by
  // the time a visitor can act on it); a recent-search resubmit doesn't wait
  // for its own fetch before navigating, so it has none to report.
  function commitSearch(term: string, opts?: { newTab?: boolean; resultsCount?: number }) {
    const q = term.trim();
    trackEvent("search_submitted", { query: q, variant, results_count: opts?.resultsCount });
  ...
  function submit(e: React.FormEvent) {
    e.preventDefault();
    commitSearch(value, { resultsCount: results.length + sealed.length });
  }
  ...
  function onKeyDown(e: React.KeyboardEvent<HTMLInputElement>) {
    ...
    if (e.key === "ArrowDown") {
      ...
      if (activeSuggestion.kind === "card") activateCardLike(activeSuggestion.card, activeSuggestion.rank, "card", newTab);
        ...
        trackEvent("search_suggestion_selected", { suggestion_rank: activeSuggestion.rank, result_type: "recent", query: activeSuggestion.term, variant });
                ...
                return (
                  <li key={`recent-${term}`} role="presentation">
                    <button
                      ...
                      tabIndex={-1}
                      onClick={() => {
                        trackEvent("search_suggestion_selected", { suggestion_rank: idx + 1, result_type: "recent", query: term, variant });
                        commitSearch(term);
                      }}
