...
export const metadata: Metadata = {
  ...
  description:
    "Free Riftbound mini-games: Riftle (the daily guess-the-card puzzle), a pack opening simulator, Riftbound 2048, Card Smash, Card Rain, Higher or Lower with live card prices, Price Check, the Zoomed In art quiz and Pairs memory. No signup, endlessly replayable.",
  keywords: [
    "Riftbound games",
    "Riftle",
  ...
  openGraph: {
    ...
    description:
      "Riftle, Higher or Lower, Price Check, Zoomed In, Pairs and Card Rain — free Riftbound mini-games built on live card data.",
    url: `${SITE_URL}/games`,
  },
};
...
export default async function GamesPage() {
  ...
  const itemListLd = {
    ...
    description:
      "Free Riftbound mini-games built on live card data — Riftle, Higher or Lower, Price Check, Zoomed In, Pairs, Pack Opening Simulator, Riftbound 2048, Card Smash and Card Rain.",
    url: `${SITE_URL}/games`,
            ...
            {deals.map((m) => (
              <Link key={m.card.id} href={cardHref(m.card)} className="card-surface group p-2.5 transition-[transform,box-shadow,border-color] duration-base motion-safe:hover:-translate-y-0.5 hover:border-ink-600 hover:shadow-glow focus-within:border-brand-500/60 active:translate-y-0">
                {m.card.imageThumbUrl && (
                  ...
                  <img {...cardThumbProps({ imageThumbUrl: m.card.imageThumbUrl }, "(max-width: 640px) 45vw, 200px")} alt={cardImageAlt(m.card)} width={300} height={420} loading="lazy" decoding="async" className="aspect-[5/7] w-full rounded-md object-cover" />
                )}
