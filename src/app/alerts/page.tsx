...
const FAQS = [
  {
    ...
    a: `Open the card's page or its quick view and tap the watch button — a free alert needs no price. We check the cheapest Near Mint (or unstated-condition) copy at the stores we track once a day, after the morning price update. You hear when it falls by at least 5% (and at least 50 cents or pence) from the price we last emailed you — or, if we haven't emailed you about that card in the last 30 days, from where it stood before it started falling, so a slow slide in small steps still adds up to an alert. If no store has the card yet, we email you when it's first listed instead, and if it sells out we email you when it's back. Plus members can also type their own price on up to ${PLUS_TARGET_ALERT_LIMIT} watched cards (every card on Premium).`,
  },
  {
  ...
  },
  {
    ...
    a: "The cheapest Near Mint copy — or one whose store states no condition — in stock at a store we track for your market (or CardTrader, or a real TCGplayer listing in the US), seen by our last price update. eBay listings never trigger an alert, and neither do played copies or a store whose feed has gone quiet (while a cheaper store's feed is down, we wait rather than guess). A drop has to be at least 5% and at least 50 cents or pence below the price we last emailed you (or, after 30 days, below where it stood before it started falling), so a few cents of drift never sends an email. A new low more than 40% under the day before is checked again at the next price update before we send it, and sent only if it is still there, because a price that far off is usually a listing error. That is the item price — postage is on top — so every alert lists up to three stores with their postage and the delivered total, and links each listing. There are no reminders: a price that just sits still doesn't email you again.",
  },
  {
  ...
  },
  {
    ...
    a: "For each card: what changed, in money and as a percentage, measured from the price we last emailed you (or where it stood before it started falling); the price when you started watching; the condition (Near Mint, or not stated by the store); up to three stores, each with its price, its postage to your market (measured at the store's checkout, or marked 'est.' where we haven't measured it) and the delivered total, and a Buy button; and when we checked. A card that's back in stock says since when it was sold out; a pre-order says when the set ships; a below-market alert shows the TCGplayer market price (converted from US dollars outside the US) and the gap. The card link opens the card in the market you're watching it in. The biggest news comes first.",
  },
  {
    q: "How do I stop, snooze or pause alert emails?",
    a: "Every card in an alert email has one-tap links: 'Stop watching' removes that card, and 'Snooze 30 days' stops emails about it for a month while we keep checking its price. Plus and Premium members also get a one-tap target 10% under the price in the email (or under their current target, if that is lower). Each link opens a short confirmation page first, so a mail app scanning links can't change anything. The footer's 'Pause alert emails' stops every price-alert email to your address and keeps your watchlist and targets; your inbox's own Unsubscribe button does the same. While paused or snoozed we still check prices, but nothing piles up: when you resume, you hear about news from then on, not a backlog. You can resume from the same page or from your watchlist. Deleting all your watches is a separate, explicit button on that page.",
  },
  {
