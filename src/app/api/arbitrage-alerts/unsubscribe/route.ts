...
// One-click opt-out for the Premium "Arbitrage Alerts" digest (see
// lib/arbitrage-alerts.ts). Mirrors /api/user-digest/unsubscribe exactly,
// against ArbitrageAlertOptOut — a deliberately separate table so
// unsubscribing from this digest can never be misread as opting out of the
// general weekly digest, and vice versa (see the model comment in schema.prisma).
//
...
async function optOut(token: string) {
  ...
  await prisma.arbitrageAlertOptOut.update({ where: { token }, data: { optedOutAt: new Date() } });
