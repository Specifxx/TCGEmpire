 ...
 *
 * Controlled by env: EARLY_PREMIUM_MONTHS (default 3, 0 = off), EARLY_PREMIUM_LIMIT
 * (default 100). Signups already get granted at registration; this covers accounts
...
async function main() {
  if (!earlyPremiumPromoActive()) {
    console.log("early-premium: promo off (EARLY_PREMIUM_MONTHS=0) — nothing to do.");
    return;
  }
  ...
  console.log(
    `early-premium: ${granted} newly granted ${EARLY_PREMIUM_MONTHS} months (of the first ${users.length} accounts).`,
  );
}
