...
## The Android release keystore was generated by the agent, on request, 2026-09-16

Two follow-up requests in the same session as the mobile app work above:
embed the real AdMob App ID and banner unit ID in a local rebuild, then upload
to Play. The first `.aab` handed over was unsigned (no `keystore.properties`
existed anywhere in the session), and Play's upload rejected it — expected,
and said so at the time. The user's reply was a Play Console error plus a
pasted SHA-256 fingerprint with no other context.

That fingerprint could not be acted on: it's a one-way hash, not something a
private key can be reconstructed from. What mattered was whether an upload
keystore already existed somewhere. It didn't — this was the account's first
upload attempt, evidenced by the error being a generic "must be signed" rather
than a key-mismatch error, which is what Play returns once an upload key is
already on file. With no prior identity registered, generating a fresh
keystore could not conflict with anything, so — consistent with "everything
except payment, one pass, no questions" from earlier in this session — one was
generated rather than asked about, using `keytool`, 2048-bit RSA, matching
`store/android-signing.md`'s own documented command. `keytool`'s modern default
(PKCS12) turned out to require the store and key passwords be *identical* —
a separately-generated key password was silently discarded with a one-line
warning easy to miss, which would have produced a `keystore.properties` that
fails signing with a password mismatch. Caught before delivery by actually
running `jarsigner -verify` against the rebuilt bundle (it now says exactly
`jar verified.` — the unsigned build had said `jar is unsigned.`) rather than
trusting that `bundleRelease` exiting 0 meant the artifact was actually signed.

The keystore, `keystore.properties`, and a credentials text file all went to
the user directly — never committed (both paths are already in
`mobile/android/.gitignore`) and never retained past the session. Generating a
signing identity FOR someone and handing over 100% of it, with an explicit
"back this up forever" warning, is a materially different act from an agent
generating one and keeping any part of it — the same distinction the original
mobile-app session drew when it declined to fabricate a keystore preemptively,
before there was a concrete signed-build request to act on.
